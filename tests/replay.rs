...
fn every_world_config_slice_reaches_the_game_config() {
    ...
    // land in `fracture:` (read by `bevy_carnage`'s bake) and the rest in `gore:`. Reading only one
    ...
    assert_eq!(
        GoreDynamics::from_config(
            ...
            app.world().resource::<bevy_carnage::FractureSettings>(),
        ),
        w.gore,
        "a plugin cloned its config slice BEFORE the seam wrote it — the seam-before-plugins ordering \
         regressed. `GorePlugin` clones `gc.gore` and `carnage::CarnagePlugin` clones `gc.fracture`, \
