## ISSUES
<!-- Doorway jamming removed 2026-07-26: does not reproduce. `tests/nav.rs` drives all five units
     through a 1-tile doorway and they traverse single-file. The wall widening (0.6 -> 0.72 m clear)
     appears to have fixed it. If it recurs, the untested case is opposing traffic through one opening. -->
...
- **Numbers kill.** The bite scales **super-linearly** with the number of crabs on a target (`dps · count^exp`), so one or two barely register and a pile shreds a unit in seconds. There is deliberately **no hard threshold** — a crab that did literally nothing would read as broken, and the exponent already makes density the threat. They cling to the **back**, out of the gun's reach, spread over the whole body.
- **Pounce.** Near a unit, hunker then leap a ballistic arc (~10 body lengths), biting on landing. Damage follows the same super-linear curve as the contact bite — a lone leaper lands and hurts a little; a wave of them is what kills.
