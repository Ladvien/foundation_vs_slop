...
/// Size this run's water field to its dungeon.
fn size_almond_field(mut commands: Commands, dungeon: Res<Dungeon>) {
    commands.insert_resource(AlmondWater::new(dungeon.width, dungeon.height));
}
...
impl Plugin for AlmondWaterPlugin {
    fn build(&self, app: &mut App) {
        // Sized and baked per run — see the note in `fog::FogPlugin` (FVS-A-5).
        app.add_systems(
            OnEnter(crate::session::RunState::Active),
            (
                size_almond_field.in_set(crate::session::RunBuild::Grids),
                bake_almond_sources.in_set(crate::session::RunBuild::Populate),
            ),
        )
