...
impl Plugin for MyceliaPlugin {
    fn build(&self, app: &mut App) {
        ...
        // `setup_habitat` reads the `Dungeon`, so the whole chain is per-run (FVS-A-5). Cosmetic/GPU
        // and windowed-only, so it never enters the harness or the deterministic core.
        // `setup_mycelia` binds the control texture into the floor material and seeds the agents inside
        // the habitat mask, so the ordering here is a real dependency, not tidiness: control texture →
        // habitat mask → agents. `MoldControlPlugin` owns `setup_control`, so this chain names it.
        .add_systems(
            OnEnter(crate::session::RunState::Active),
            (setup_habitat, setup_mycelia)
                .chain()
        ...
        // module that touches pinned state, and naming it here is what makes that visible.
        .add_plugins((
            control::MoldControlPlugin,
            fruit::FruitPlugin,
            grazing::GrazingPlugin,
            measure::MoldMeasurePlugin,
            testbed::MoldTestbedPlugin,
        ));
