...
/// Fruit bodies — the mold reproducing.
///
/// Its own plugin rather than an inlined `build(app)` call so the boundary is declared, not implied:
/// it owns `MoldCoarse`/`PinDwell` and the fruit material, and depends on the parent's textures and
/// config. Cosmetic/GPU, windowed-only.
