...
pub fn squad_think(
    ...
    let unit_snapshot: Vec<(Entity, Vec3, f32)> = units
        .iter()
        .map(|(e, t, _, _, _, _, _, h, _)| (e, t.translation, health_frac(h)))
        .collect();

    let dt = time.delta_secs();

    for (entity, tf, role, mut drives, mut active, mut timer, mut desired, health, order) in
        &mut units
    {
        ...
        let ai_drives_this_unit = order.is_none()
            && match *control {
                ...
                // ControlOne hands the leader to the player; distinguishing the leader needs a
                // `PlayerControlled` marker (follow-up). Until then it behaves like the others.
                SquadControlMode::ControlOne => true,
            };
...
fn resolve_goal(mode: Mode, perc: &Perception, anchor: &SquadAnchor) -> Option<Vec3> {
    ...
    match mode {
        // Cohesion: head for the group reference point.
        Mode::Regroup | Mode::FollowAnchor => anchor_goal,
        // Retreat toward the squad when afraid (regroup to safety).
        Mode::Flee => anchor_goal,
