...
fn despawn_dead_nests(
    mut commands: Commands,
    nests: Query<(Entity, &Transform, &Health), With<Nest>>,
) {
    let mut dead: Vec<(Vec3, Entity)> = nests
        .iter()
        .filter(|(_, _, hp)| hp.current <= 0.0)
        .map(|(e, tf, _)| (tf.translation, e))
        .collect();
    // Nest query order is not stable across `App` instances; sort so despawn order (and the entity-id
    // reuse it drives) is reproducible if two nests are ever razed on the same tick — matching
    // `crab_despawn_dead` (`CrabSeed`) / `despawn_dead_units` (`SquadMember`). The key is the nest's
    // POSITION, the stable identity a stationary structure carries: nests are placed one per distinct
    // site, so it cannot tie — and `sort_total!` panics naming this site if that ever stops being true.
    // Entity bits would NOT do here: id recycling is run-dependent, the very instability being erased.
    crate::sort_total!(&mut dead, |d: &(Vec3, Entity)| {
        (d.0.x.to_bits(), d.0.y.to_bits(), d.0.z.to_bits())
    });
    for (_, e) in dead {
