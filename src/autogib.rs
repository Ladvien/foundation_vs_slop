//! # Autogib — the game's half of `bevy_autogib`
//!
//! The fracture itself — the triangle soup, the recursive plane cuts, the watertight caps, the
//! bake-once-per-source cache — lives in `crates/bevy_autogib`, which knows nothing about this game.
...
//! `crates/bevy_autogib/src/bake.rs`, with `crates/bevy_autogib/CLAUDE.md` carrying the summary. That
...
///
/// `AutogibCache` is `bevy_autogib::FractureCache`, and `GunChunk` is its `DetachedChunk` — the crate
/// must not say "gun", because a fracture library is useless to a project that has none.
pub use bevy_autogib::{DetachedChunk as GunChunk, Fragment, FractureCache as AutogibCache};
...
/// authored [`FractureSettings`](bevy_autogib::FractureSettings) from `config.ron`, and the run gate.
/// `lib.rs` and `sim_harness.rs` both add `autogib::AutogibPlugin` and neither needed to change.
...
impl Plugin for AutogibPlugin {
    fn build(&self, app: &mut App) {
        ...
        //
        // Inserted BEFORE the crate's plugin on purpose: `AutogibPlugin` there `init_resource`s
        ...
        app.insert_resource(settings)
            .add_plugins(bevy_autogib::AutogibPlugin)
            // The crate deliberately configures no run condition — the caller owns the schedule.
            .configure_sets(
                Update,
                bevy_autogib::AutogibSystems.run_if(in_state(crate::session::RunState::Active)),
            )
            ...
            .add_systems(
                Update,
                tag_valkyrie_rifle
                    .before(bevy_autogib::AutogibSystems)
...
/// still streaming" gate stays satisfied. Runs `.before(AutogibSystems)` so the tag is in place the same
