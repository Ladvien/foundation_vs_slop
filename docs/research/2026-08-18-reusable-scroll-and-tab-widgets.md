...
- **`bevy_input_focus`**: `InputFocus`, `InputFocusVisible`, `TabIndex(i32)`, `TabGroup { order, modal }`,
  ...
  `FocusedInput<T>`.
- **`AcquireFocus` bubbles until it hits something with `TabIndex`** (`acquire_focus` queries
  `Query<(), With<TabIndex>>`). Clicking a caption therefore focuses the nearest `TabIndex`-bearing
  *ancestor* — which, in a roving-tabindex strip where the tabs themselves carry none, is the strip. §5.3.
...
- **`Selectable` and `Selected` live in `bevy::ui`, not `bevy_ui_widgets`** — `list.rs` imports them from
  there. `ListItem` is `#[require(.., Selectable)]`; `Selected` is present only while a row is selected.
- **AccessKit roles we need all exist** in `accesskit` 0.24: `ScrollView`, `ScrollBar`, `TabList`, `Tab`,
  ...
  Note `set_selected()` takes **no argument** — it is a flag setter; clearing is `clear_selected()`.
...
- **`FeathersListView` / `FeathersListRow`** — `ScrollArea` + overlay `FeathersScrollbar` + `ListBox` +
  `Role::ListBox` + `TabIndex(0)`, rows carrying `Role::ListItem`, `Hovered`, `ListItem`
  (which itself requires `Selectable`), and `Selected` once chosen.
...
// `SceneComponent` requires `type Props: Default`, and `Box<dyn SceneList>` has no blanket `Default`.
// Upstream hand-writes exactly this for `FeathersListViewProps`; so must every props struct here.
impl Default for ScrollViewProps {
    fn default() -> Self {
        Self {
            content: Box::new(bsn_list!()),
            sticky:  Box::new(bsn_list!()),
            axis: ScrollAxis::Y,
            gutter: Gutter::Overlay,
            bar: BarVisibility::Auto,
            row_gap: px(2),
            keys: None,
        }
    }
...
/// `BarVisibility` carried onto the bar entity — props are not readable after spawn.
#[derive(Component)] pub struct BarPolicy(pub BarVisibility);
```
    ...
    fn scene(props: ScrollViewProps) -> impl Scene {
        ...
        bsn! {
            ...
            Node {
                ...
                flex_grow: 1.0,
                min_height: px(0),
                // Braced: `bsn!` values are literals, paths, calls or `{ expr }` — a bare struct
                // literal with `..default()` is not a bsn! value form.
...
impl Plugin for EditorWidgetsPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(FeathersPlugins)                  // TabNavigationPlugin + FeathersCorePlugin
           ...
           .add_observer(scroll_chain);      // §3.6 — replaces `ScrollArea` on nested views
        // `ScrollMomentum`'s integrator (§3.7) joins the `PostUpdate` tuple once it lands, behind its flag.
    }
}
```
...
- **`FeathersPlugins` already includes `TabNavigationPlugin`.** Adding it twice is a duplicate-observer bug,
  not a no-op.
- **Only the Tab-*key* observer is window-scoped.** `setup_tab_navigation` runs at `Startup` and observes
  `handle_tab_navigation` on the primary window; `acquire_focus` and `click_to_focus` are global
  `app.add_observer`s. So in `build_headless` (`WgpuSettings { backends: None }`, no window) **Tab-key
  traversal silently does nothing**, while click-to-focus and `AcquireFocus` still work. Headless assertions
  about Tab order must drive `TabNavigation::navigate` / `InputFocus` directly rather than synthesising a
  Tab key.
...
- **A missing `Res<T>` panics its system in Bevy 0.19**, and all run conditions are evaluated —
  `evaluate_and_fold_conditions` folds with `acc && res` and deliberately does not short-circuit. Every
  system above takes `Option<Res<..>>` or is `init_resource`'d. Related 0.19 change worth knowing while
  writing them: the `and`/`or`/`nand`/`nor` combinators are **deprecated since 0.19.0** in favour of
  `and_then`/`or_else` (short-circuiting) and `and_eager`/`or_eager`.
...
Every claim above was put through an adversarial verification pass against the 0.19.1 docs and sources before
this document was filed; the corrections it produced are already folded in (`Selectable`/`Selected` live in
`bevy::ui` not `bevy_ui_widgets`; only the Tab-*key* observer is window-scoped, so headless click-to-focus
still works; props structs need hand-written `Default` impls; `set_selected()` takes no argument). What that
pass could **not** do is check the 0.19.0 pin.

So the rule applies to this document too. **First implementation task: diff this document's API surface
against the vendored 0.19.0 sources** and correct or annotate anything that moved. Highest-risk items, because they
are newest:
