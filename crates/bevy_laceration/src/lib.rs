...
/// The anatomy this crate cuts through, re-exported whole so a consumer needs one dependency line
/// rather than two that could drift in version.
pub use bevy_cross_section;
/// The blood model the cross-section's muscle band is coloured by, re-exported for the same reason.
pub use bloodstain;
...
pub use bevy_cross_section::{Layers, Region, Scale};
...
use bevy_cross_section::{CrossSectionAtlas, CrossSectionSettings};
