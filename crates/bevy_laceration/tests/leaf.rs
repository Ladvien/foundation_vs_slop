...
//!
//! `bevy_laceration` is a time curve, a mesh-cutting kernel and one plugin. It takes `bevy` —
//! trimmed to the asset arena, the mesh types, `MeshMaterial3d` and the logger — `bevy_cross_section`
//! for the anatomy the wound bed is banded by, and `bloodstain` for `hash_f32`, the one frozen random
//! source this family shares. Nothing else.
//!
//! **`bevy_carnage` is forbidden by name because the layering runs the other way**: it composes this
//! crate, opening wounds on bodies it has already fractured. A dependency in this direction would be
//! a cycle. `bevy_hanabi` and `wgpu` would move the cut onto the GPU, which is the one thing a
//! hashable tear cannot allow — `digest` exists precisely because the geometry is computed where it
//! can be read back. `avian`, `emerge` and `foundation_vs_slop` are the game: a laceration must be
//! authored by its caller, not derived from a physics solve this crate would then be pinned to.
//!
...
/// Everything `bevy_laceration` is allowed to depend on.
...
/// would move the cut onto the GPU, where it could not be hashed; `avian`, `emerge` and
/// `foundation_vs_slop` because they are the game.
...
fn no_source_file_reaches_across_the_layering() {
    ...
    assert!(
        offenders.is_empty(),
        "bevy_laceration must stay below the gore layer and off the GPU, but {} line(s) reference a \
         forbidden crate.\n  {}\n\n\
         `bevy_carnage` composes this crate, so a dependency the other way inverts the layering. A \
         compute or physics dependency would move the cut onto the GPU or make its shape a \
         consequence of a solver, and either one ends the property `digest` exists to prove: that a \
         tear is a pure function of its arguments.",
        offenders.len(),
        offenders.join("\n  ")
    );
}
...
fn the_dependency_list_stays_closed() {
    let manifest = std::fs::read_to_string(crate_root().join("Cargo.toml"))
        .expect("bevy_laceration must have a Cargo.toml");
    ...
    assert!(!deps.trim().is_empty(), "bevy_laceration must declare a [dependencies] table");
    ...
    for line in deps.lines() {
        ...
        assert!(
            ...
            "bevy_laceration declares `{name}`, which is not in its allowed set {ALLOWED_DEPS:?}.\n\
        ...
        for marker in FORBIDDEN_DEP_MARKERS {
            assert!(
                ...
                "bevy_laceration declares `{name}` — see this file's header for why that name in \
