...
pub struct WetSettings {
    ...
    /// what makes the drip pass conserve mass exactly (see [`crate::WetCanvas::tick`]).
    pub drip_rate: f32,
    ...
    ///
    /// **The same reference `bloodstain::dry::DRY_REF_TICKS` uses**, so blood on a wall and blood on a
    /// floor dry at one rate. It is the single authority: a texel's age is rescaled onto that
    /// reference before `bloodstain::dry::appearance` is asked what the blood looks like, so moving
    ...
    /// That budget is why the default canvas size is 128 rather than 512 — see [`crate::WetCanvas::new`].
    pub max_canvas_updates_per_tick: u32,
    /// Relative humidity, `[0, 1]`. `0.4`.
    ///
    /// Forwarded straight into a [`BloodSettings`] for `bloodstain::dry::appearance`, which grows a
...
impl WetSettings {
    ...
    /// The [`BloodSettings`] this crate hands `bloodstain::dry::appearance`.
    ///
