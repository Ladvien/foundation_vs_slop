...
fn main() {
    ...
    println!(
        "\n  (Area is quoted under bevy_wetmap::UV_SPAN_M = {:.1} m per UV unit; a texel is \
         {:.3} mm across.)",
        bevy_wetmap::UV_SPAN_M,
        bevy_wetmap::UV_SPAN_M / SIZE as f32 * 1000.0
    );
}
...
///
/// Built by hand rather than through `bloodstain::stain::stain_shape` so this example has exactly one
