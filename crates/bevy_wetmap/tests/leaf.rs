...
//!
//! `bevy_wetmap` is a texel buffer, four passes over it, and one upload system. It takes `bevy` —
//! trimmed to five features, because it needs the asset arena, the mesh types, three `wgpu` type names
//! and a logger — and `bloodstain`, which owns the blood model it paints with. Nothing else.
//!
//! That matters for a specific reason. **The reason this crate exists is that its state never leaves
//! the CPU**: a GPU render target cannot be hashed, and every other texture-space wetmap is one. A
//! dependency that brought a compute path, a physics engine, or a game type in would either put the
//! authority back on the GPU or make the crate unusable outside the game it was carved out of.
//!
//! `bevy_hanabi`, `wgpu` and `avian` are forbidden by name for that reason and not for tidiness. So is
//! `bevy_carnage`: it **composes** this crate, and a dependency the other way would invert the
//! layering — which is also why `src/uv.rs` reimplements Möller–Trumbore instead of importing it.
//!
...
/// Everything `bevy_wetmap` is allowed to depend on.
