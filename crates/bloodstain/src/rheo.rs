...
///
/// **Monotone, and exact at both ends.** The same integer-tick ramp shape [`crate::bleed::flow`]
...
mod tests {
    ...
    fn flow_arrests_inside_the_taper() {
        let s = BloodSettings::default();
        let driving = |age: u32| PERFUSION_STRESS_PA * crate::bleed::envelope(age, &s);
    ...
    fn an_arrested_flow_never_resumes() {
        ...
        for age in 0..=(s.clot_ticks + 600) {
            let f = flows(PERFUSION_STRESS_PA * crate::bleed::envelope(age, &s), yield_stress(age, 60, &s));
            if arrested {
