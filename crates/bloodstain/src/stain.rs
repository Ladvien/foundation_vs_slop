...
use crate::{V3, Wound, hash_f32, m, vec};
...
pub struct Impact {
    ...
    /// [`crate::origin::area_of_origin`] — and a value a caller has to remember to overwrite is a
...
///
/// `viscosity` is the caller's, from [`crate::rheo::viscosity`] at the shear rate the impact implies —
...
pub fn stain_shape(i: &Impact, s: &BloodSettings, seed: u32) -> StainShape {
    ...
    let mu = crate::rheo::viscosity(i.speed / i.diameter.max(1.0e-6), s.hematocrit, s);
...
impl StainShape {
    ...
    ///
    /// **The inverse of the forward model, and the whole basis of [`crate::origin`].** A stain that
...
/// consumer's material carries the blood colour and [`crate::dry::appearance`] walks it, so one mask
...
mod tests {
    ...
    /// move was a move. Read [`crate::droplet::tests::the_spatter_model_is_frozen`] for the two
    ...
    fn the_aspect_ratio_is_the_sine_of_the_impact_angle() {
        ...
        for deg in [15.0f32, 30.0, 45.0, 60.0, 75.0, 90.0] {
            let i = Impact {
                speed: 5.0,
                diameter: 0.004,
                angle_rad: crate::to_radians(deg),
                roughness: 0.0,
                travel: [1.0, 0.0],
            };
            let shape = stain_shape(&i, &s, 0x1234);
            let expect = m::sin(crate::to_radians(deg));
    ...
    fn the_mask_is_reproducible_and_not_empty() {
        ...
        let shape = stain_shape(
            &Impact { speed: 8.0, diameter: 0.004, angle_rad: crate::to_radians(40.0), roughness: 0.1, travel: [1.0, 0.0] },
            &s,
            0xBEEF,
        );
        let px = 48u32;
    ...
    fn a_shallow_impact_rasterises_an_elongated_stain() {
        let s = BloodSettings::default();
        let shape = stain_shape(
            &Impact { speed: 6.0, diameter: 0.004, angle_rad: crate::to_radians(15.0), roughness: 0.0, travel: [1.0, 0.0] },
            &s,
            1,
        );
        let px = 64usize;
