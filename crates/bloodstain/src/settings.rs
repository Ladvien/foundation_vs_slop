...
pub struct BloodSettings {
    ...
    ///
    /// **A tuning dial, not a measured law** — see [`crate::rheo::viscosity`], which says why in as
    ...
    pub hematocrit: f32,
    /// Yield stress at a full clot, Pa. [`crate::rheo::flows`] fails against it, which is what makes
    ...
    /// `doi:10.1007/s00414-010-0498-5`) — so [`crate::patterns::expirated`] usually returns zero
    /// rings, deliberately.
    ...
    /// budget in [`crate::patterns::transfer`].
    ...
    /// Default substrate roughness fed to [`crate::stain::stain_shape`]. A rough surface shortens a
