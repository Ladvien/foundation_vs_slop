...
//! [`area_of_origin`] cannot recover a wound from the stains [`crate::stain::stains`] produced, then
...
mod tests {
    use super::*;
    use crate::stain::{Impact, stain_shape};
    use crate::{BloodSettings, vec};
    ...
    fn the_solver_recovers_a_known_wound() {
        ...
        for i in 0..200u32 {
            ...
            let elevation = crate::to_radians(-10.0 + 40.0 * crate::hash_f32(i));
            let speed = 4.0 + 4.0 * crate::hash_f32(i ^ 0x5EED);
    ...
    fn the_tangent_method_alone_would_overestimate_the_height() {
        ...
        for i in 0..120u32 {
            ...
            if let Some(l) = thrown(truth, azimuth, 6.0, crate::to_radians(5.0), gravity) {
                landings.push(l);
            }
        }
        let with_gravity =
