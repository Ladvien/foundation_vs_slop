# BACKLOG_ARCHIVE.md — completed items

Everything in `BACKLOG.md` that reached ✅, moved here so the backlog shows only what is left. Item IDs
(`FVS-<epic>-<n>`) are stable and are **not** reused — an ID appearing here will never appear in the
backlog again. Push headings are preserved so each item keeps the context it was scoped in.

Split out 2026-07-30.
...
- **FVS-C-1 — Wire SCP-610 from zero** · L (+ an asset export first) · *determinism: core* · ✅ **LANDED 2026-07-30 — v1, stationary**
  The asset blocker is gone (it landed 2026-07-29 and was recompressed under N-19). `src/scp610/` follows the `scp999::spawn_scp999_at` shared-builder contract, splits gameplay/visuals across two plugins, and appends `knowledge::Subject::Flesh` with its gossip lines, research entry and a `contact_flesh` conversation.
  **Scope, stated rather than implied: v1 does not move.** No `BrainId`, no `Drives`, no locomotion, no attack — it is terrain that is alive, which is canon (infected root themselves) and which keeps it out of four append-only enums whose discriminants index archived RL policies. It carries `Faction::Anomaly`, whose drive list is deliberately empty.
  *Two defects found only by looking, with every test green:* the `mutation` morph wrote to target index 1, which does not exist in glTF (`Basis` is the base mesh, not a target) — three blooms authored at 0.0/0.5/1.0 rendered identically; and the raster spawner took the first N eligible cells from (0,0), clustering every bloom in the map's low corner. Both fixed; placement is now one bloom per evenly-spaced room, pinned by a test on *spread* rather than mechanism. · *Deps:* B-6 · *Reading:* [ECS]
...
- **FVS-N-17 — The GOC type matrix was dead code, duplicated in three places** · S · ✅ **LANDED 2026-07-30**
  `src/palette.rs` exists to be the single source of truth for every non-UI colour and was failing at it: `psi_vision::PSI_GROUP_HUES` hardcoded Magenta and Green as `[f32; 3]` literals, and `CRAB_ICHOR` re-spelled Gray-Green — each a few lines from the constant it duplicated, each *commented as though it referenced it* (`crab/combat.rs` says "Type-Gray reanimated ichor" and references neither). All three would have survived a hue change and silently disagreed afterwards. Now aliased; `the_goc_matrix_has_no_duplicated_literals` pins the aliasing, not the values.
- **FVS-N-18 — `with_tangents` panicked; now a fallible system** · S · ✅ **LANDED 2026-07-30**
  Argued from repo convention (`sort_total!` panics naming its site) and overruled: `CLAUDE.md`'s "no panics" is unconditional and a convention is not an exemption. `spawn_tiles` returns `Result<(), BevyError>`, the pattern `mold.rs:298` already uses. Deliberately **not** a degraded path — if it fired the dungeon would not spawn and Bevy would log why, which is loud; carrying on with untangented meshes would silently disable every normal map and merely look "a bit flat".
- **FVS-N-19 — SCP-610 asset recompressed, 28.7 → 5.2 MB, geometry untouched** · S · ✅ **LANDED 2026-07-30**
  88% of the file was ONE 2048² **16-bit** PNG normal map — 35% of the entire `assets/` tree for a creature that spawned nowhere. `scripts/glb_recompress_texture.py` rewrites the GLB container rather than round-tripping through Blender, specifically to protect the contract the hand-off doc pins (13 bones, five clips **in order**, the `Basis`/`mutation` morph pair): an exporter is free to reorder animations or retriangulate. It refuses on overlapping bufferViews, 4-byte-aligns, pads per spec, then re-parses its own output and aborts unless every count and name matches.
- **FVS-N-20 — Three defects in the Blender converters, each found by measurement** · S · ✅ **LANDED 2026-07-30**
  (a) **Parented objects exported unrotated** — `transform_apply` bakes only an object's own transform, and glTF imports arrive parented to a root empty carrying the Y-up→Z-up conversion, so the barrels came out lying on their side and base-at-origin then planted a *side* face at y=0. (b) **Junk inherited scale** — that same chain scales 0.01 then 334.94 (a Sketchfab round-trip), yielding 2.87 m barrels; caught only by knowing a 55-gallon drum is 0.851 m. (c) **Unspecified object ordering** — both scripts iterated `bpy.data.objects` and `import_retro_tvs.py` applied names *positionally*, so a reorder would pair a filename with a different mesh while the manifest kept its measured footprint. All three would have shipped silently.
- **FVS-B-9 — The quarantine verb is a no-op: nothing inserts `Quarantinable` (FOUND 2026-07-28, review)** · M · ✅ **LANDED 2026-07-30**
  Closed by C-1. `src/scp610/` inserts `Quarantinable` at spawn, so `containment::area::tick_quarantine` — which had existed, been tested, and run on nothing — now has a subject. No change to the quarantine code itself was needed, which is the point: the archetype was correct and merely unused.
...
- **FVS-Q-1 — The relight: HDR + bloom, irradiance environment map, the game's first shadows** · L · ✅ **LANDED 2026-07-30**
  ...
  Shadows: the directional key only. Hubona, Wheeler, Shirah & Brandt 1999 (`10.1145/329693.329695`) found cast shadows improve object-positioning accuracy — the judgement this game asks — but that performance *degrades* going from one shadowing source to two. `key_illuminance` 2000 → 6000, because key-vs-fill **is** the contrast ratio of every shadow on screen.
  *Two things this exposed:* VHS and bloom both defaulted into `Core3dSystems::PostProcess` with only a `.before(tonemapping)` edge, leaving their order unconstrained once bloom existed (VHS is now explicitly `.after(bloom)` — glare is what the scene does to light, tape damage is what the medium does to the finished picture); and **22 sites needed `NotShadowCaster`**, because turning shadows on made every billboard, decal and unlit overlay a caster. Do not assume alpha handles that — Bevy 0.19's shadow queue does not skip blended materials, it only tags them `MAY_DISCARD`. · *Verified:* by capture, not inference · *Reading:* [RENDER]
- **FVS-Q-2 — Derived normal + ORM maps for the Backrooms surfaces** · M · ✅ **LANDED 2026-07-30**
  `assets/textures/` shipped two textures, both diffuse-only. Under the old uniform ambient that did not matter — a normal map cannot show when every surface gets the same fill whatever its normal — which is why the project had none. Q-1 made relief read, so `scripts/derive_surface_maps.py` infers it from linear-light luminance, stating plainly in its own docstring that luminance is *not* height. Wrap-around gradients (both inputs tile every metre; clamped edges would put a bright line on every tile boundary — the exact grid the relight was meant to stop advertising) and a `--height-blur` that separates printed pattern from physical relief, after a first attempt at strength 2.4 turned the wallpaper into sandpaper.
  Bevy gates normal mapping on `ATTRIBUTE_TANGENT`, which its primitives do not emit — without generating tangents every map would have loaded, cost memory and changed nothing on screen, with no error anywhere. Pinned by `tangents_exist_for_every_dungeon_mesh`. · *Reading:* [RENDER]
- **FVS-Q-3 — Surface biomes: Backrooms motel and Foundation concrete** · M · ✅ **LANDED 2026-07-30**
  Two art treatments over one geometry so a run reads as moving between zones, and so the concrete side can be the *desaturated counterweight* the colour doctrine wants against authored-yellow wallpaper.
  **Biome is a pure function of `(seed, cell)`, and that is the design.** Determinism becomes structural — nothing for query order to decide, so no `sort_total!` and no key to get wrong; rooms and corridors agree at doorways for free; and it draws nothing from the carve RNG, so `tests/wfc_pin.rs` confirms the layout is byte-identical. `FloorMaterials` became per-biome behind a `pick()` accessor, because fog swaps floor materials constantly and a caller who remembered fog but forgot biome would repaint concrete as carpet the moment a unit looked at it. · *Reading:* [RENDER]
- **FVS-Q-4 — Asset imports: retro CRTs, Tiny Living furniture, hazard drums, Ozea floor plates** · M · ✅ **LANDED 2026-07-30**
  Rooms were empty boxes; the relight had nothing to light. Data-only on the game side — manifest rows, zero code.
  The CRTs had to satisfy a contract that **fails silently**: `light::glow_screens` finds a screen by `green + blue > 3*red + 0.05` in linear space, and the pack's glass is neutral grey. Two non-obvious things had to be right, both measured: glTF writes `baseColorFactor = (1,1,1,1)` whenever a base-colour *texture* is linked, discarding the socket value; and `img.scale()` is discarded on export unless followed by `img.pack()` (2K→1K moved a GLB 5.32→4.93 MB, and that delta was recompression, not resolution). `scripts/import_retro_tvs.py` asserts the predicate on its own output.
  **The Ozea corridor kit was oversold and measurement caught it.** Its floors are exactly 1.00 × 1.00 m — a perfect `TILE_SIZE` match — but its walls are 2.00 m against `WALL_HEIGHT` 2.4 and 2 m wide against a 1 m grid, so only the floor plates landed. `warning-signs-set` was skipped: 29 meshes named `p_cube101`…`p_cylinder4` at 4.5 cm sharing one atlas. · *Reading:* [PLACE]
- **FVS-Q-5 — Audio: per-biome footsteps, wider growl and ambient pools** · S · ✅ **LANDED 2026-07-30**
...
- **FVS-N-24 — The 11.6 s oscillation and the 171 ms hitches were an EXTERNAL GPU TENANT (RESOLVED 2026-07-31)** · M · ✅ **NOT A GAME DEFECT — closed by measurement**
  ...
  *Done when:* the stall is named, and either fixed or shown to be unavoidable with the measurement
  ...
  > ### ✅ CLOSED 2026-07-31 16:47-16:57 — the quiet-GPU re-measure. The cycle is simply GONE.
  > The box's olmocr/vLLM pipeline was stopped (`llama-swap`, `hs-serve-scribe-olmocr`,
  > `hs-serve-distill`, the inbox + distill-event watchers), verified idle — `nvidia-smi` showed **no
  > compute apps, 1331 MiB, 0% util** (down from 20,292 MiB and 82-94%). Then 4 x 150 s runs, same
  > scene and recipe as every prior capture (~7 visible lights, cell 80,112), interleaved
  > 1280x720 / 640x360 / 1280x720 / 640x360, first 10 s discarded (n≈277 each):
  > | run | res | median | p90 | p99 | max | frame acorr |
  > |---|---|---:|---:|---:|---:|---:|
  > | Q1 | 1280x720 | **4.18 ms** | 4.25 | 4.37 | 4.52 | **+0.002** |
  > | Q2 | 640x360 | 4.28 ms | 4.35 | 4.46 | 4.58 | +0.000 |
  > | Q3 | 1280x720 | 4.21 ms | 4.29 | 4.44 | 4.47 | +0.001 |
  > | Q4 | 640x360 | 4.25 ms | 4.31 | 4.40 | 4.51 | +0.000 |
  >
  > * **The oscillation does not exist on a quiet GPU.** Frame autocorrelation is **≈0.00 in all four
  >   runs** (against +0.14 … +0.61 while the OCR engine ran), and GPU-util autocorrelation collapsed
  >   from +0.44…+0.55 to +0.01…+0.03. There is no 11.6 s cadence, no slow phase, nothing to attribute.
  > * **The 171 ms hitch class is gone too.** Worst single frame across all four runs: **4.58 ms**.
  >   The whole distribution spans 4.18 → 4.58 ms; p99 sits 0.2 ms above the median.
  > * **The game runs at ~238 fps (4.2 ms) at 720p with 412k visible triangles and 7 lights.** The
  >   "25-48 fps sustained", the "1% low of 8.7 fps", the "worst frame 171 ms" and the player's
  >   "it drops to 26 FPS" were **GPU contention from the OCR pipeline**, not this game.
  > * Frame pacing is flat enough (median→max = 0.4 ms) that the intro-conversation sim pause is not
  >   even visible in the distribution — i.e. the live sim costs no measurable frame time here either.
  *Resolution:* not a defect in this codebase. The standing requirement is **procedural**: verify the
...
- **FVS-R-7 — A grammar over compositions, behind a validity-function seam** · ✅ **LANDED 2026-08-10**
  `grammar::from_compositions` builds prototypes from the composition set, and adjacency goes through a substitutable `agrees(&Interface, &Interface, Dir) -> bool` — Karth & Smith's *"any arbitrary adjacency validity function can be substituted here… without changing the WFC solver itself"*. A test drives the learner twice with opposite rules and asserts the supports differ.
  **Four turns per tile is what made the kit solvable at all.** Measured before it existed: every wall tile's north support was `Empty` and nothing else, because a wall presents `wall` outward on one face and only the same tile turned 180° presents it back. The turned interface is **derived, not rotated** — `turn_composition` goes through the `rotate_xz`/`add_yaw` pair a stamp already turns by, so there is no second sign convention to get backwards.
  **One tile, one unit of weight, split across its turns.** Uniform per *prototype* is not uniform per *tile*: a symmetric tile dedupes to one and an asymmetric one keeps four, so a flat weight made `tile_floor` a quarter as likely as its neighbours — 7.4% of 864 cells against ~23%. Fixed, and the correction is what made the plans have interiors at all.
  **The wiring, and the type error it exposed.** `Cmd/Ctrl+G` is the third `Source`. It needed `Prototype::Composed`: `from_compositions` had been pushing a `Piece` whose `descriptor` held a *composition* id, which type-checks and then resolves against the wrong file — `library.get` finds nothing and every row is dropped while the status line reports success. The kit test agreed with the bug, asserting `placements.len() > 20` on rows that were all unresolvable. Stamps are built inside `solve` rather than by a translator over the grid, because a translator cannot see `initial` and would have to re-derive which cells the author pinned.
  *Shipped:* `grammar::{Composed, Prototype::Composed}`, `Solved::{stamps, grid, width, height}`, `Source::Composed`, two keys tests, one headless test. Seven mutations, seven killed.
- **FVS-R-9 — Expressive range, against thresholds already committed** · ✅ **RUN 2026-08-10 — and the approach is falsified**
  128 solves over a 12 × 12 region. **None failed, and none produced an enclosed region at all** — the histogram is empty. Row 1 (median enclosure < 0.15) reads **0.000** and fires; row 3, the gate, passes at 0.0% non-convergence, which is what makes the rest readable.
  Two readings of "enclosure is always 0" look identical and both were checked rather than argued: the metric **can** see a room (a hand-laid one measures 1.0, one door reads 1.0, a breach reads 0.0), and that room is **legal** under the learned support. So the grammar permits a room and the sampling never finds it.
  The mechanism is measured — see **FVS-R-17** — and two blind spots in the criterion itself are recorded in **FVS-R-18**.
  *Shipped:* `emerge_core::range` (`measure`, `bin`, `max_bin_share`, `normalised_entropy`, `total_variation`, `Faces`), `examples/expressive_range.rs`, `docs/research/2026-08-10-expressive-range.md`. Ten mutations, ten killed — three of which found real coverage gaps in the seam and fill logic.
- **FVS-R-12 — `bevy_debugger/input` cannot type** · M · ✅ **LANDED 2026-08-10**
  It wrote `ButtonInput`, which is Bevy's **fold** of the input stream rather than the stream. That reached everything reading `ButtonInput` and nothing reading `MessageReader<KeyboardInput>` — which is every text field there is, since they all match on `logical_key`. So an agent could press keys and not type into them, and could not press `Enter` or `Escape` **in** a field either. It blocked three verifications in one day.
  *Shipped:* `apply_pending_input` runs `PreUpdate.before(InputSystems)` and writes `KeyboardInput`/`MouseButtonInput` messages; `keyboard_input_system` clears last frame's edges and *then* folds, so a message written ahead of it survives the clear that erased the old direct write — the same engine fact as the first fix, used from the other side. `InputCommand` gains `text: Option<String>`, one message per character.
  **One kind, two fields; a separate `kind: "Text"` was designed and rejected.** `Escape` is spelled identically in `KeyCode` and `Key`, so a split would have made an agent choose a kind by guessing which of the *host's* systems was listening — knowledge it cannot obtain — and each kind would deliver half a key while reporting success, which is the original bug's shape. The objection that had killed this design ("no layout-independent `KeyCode → Key` mapping") was **wrong, and measuring settled it**: 194 `KeyCode` unit variants, 306 `Key` variants, **93 spelled identically**, including every named key a text field matches. `logical_for` hands the caller's own string to Bevy's `Key` deserializer — no table, no drift. `KeyW`/`Numpad7`/`ShiftLeft` answer `Unidentified`, because `KeyW` is `w` on QWERTY and `,` on Dvorak.
  Four things that cost something to find: a burst of characters needs **no** spreading (a stream has no edge to lose, so `"site_67"` is one call and one frame); every character **must** release, because `ButtonInput` is derived now and `release` is the only thing clearing `pressed`; text defers one frame behind the key that *opens* a field, since fields drain the stream while shut (the `xseam` guard) — but **nothing defers behind text**, so type-and-commit is one round trip; and a space is `Key::Space`, not `Character(" ")`, which five handlers in `emerge-mapper` would silently drop.
  *Verified end to end, in a running editor on its own `BEVY_BRP_PORT`, no human at the keyboard:* `Shift+B` → box drag → `M` → `{"kind":"Keyboard","text":"porch_a"}` → `Enter` → `porch_a` validated and **written to `compositions.ron`** as `Bounded(size: (3.0, 2.4, 5.0))` with its members. The proof is the file, not a screenshot.
  *Also:* `ordering_before_input_systems_erases_the_press_entirely` → `ordering_after_input_systems_costs_a_frame`, and that rename is a finding — misordered, an injected key now arrives a frame **late** rather than never. `tests/leaf.rs` earned its keep by catching `winit` in an assertion **string**, which `strip_comments` does not reach. All three new invariants were mutation-tested. Third example added, `typed_text_lands`, asserting like `cursor_drag_lands`.
  **Recorded for FVS-R-5:** driving this by injected pixel coordinates took five attempts, and every failure was editor-state choreography rather than input — a click stamps when a set is in hand; `Escape` dismisses the problem banner *before* it puts a set away; a third `Escape` disarms the tool so the next drag paints. That is the brittleness the corpus vetting predicted for a coordinate-replay driver, met in anger. · *Deps:* — · *Reading:* [BRP]

- **FVS-R-5 — Convert `site_67`'s architecture to stamps** · M · ✅ **LANDED 2026-08-10**
  149 hand-placed rows → **144 stamps**: 139 `site/tile_floor` and 5 `site/tile_wall_n` at yaw 90, every one of the 144 cells covered exactly once, `placements: []`. The five walls that sat **centred on the tile seam** (`x = 0.0`, spanning `[-0.05, 0.05]`) are now inset 50 mm east on cells `(0.5, 7.5)…(0.5, 11.5)` — the direction the plan recorded, because a straddling wall read as a composition sits on no face and presents nothing. Lossless, checked rather than assumed: `Stamped` has no `lift`/`tip`/`patch`/`paint` and this map used none.
  *Verified:* a captured frame shows the run **unbroken** — five panels, clean butt joints, no gap, no doubled thickness — over a single floor surface with no z-fighting, which is what replacing the floor rows rather than stamping beside them buys. `Map::validate` passed (the editor refuses to save an invalid map, and it saved). Whole workspace green, no golden moved.
  *Done entirely over BRP, in a real editor, with the machine's own keyboard and mouse untouched.*

  **Finding 1 — the tile-kind count is ZERO for this map, and that is the result rather than a null one.** Open floor and a single-face wall both have a tile, and `one_wall_tile_covers_four_orientations` covers the run's orientation. `site_67` is a 12 × 12 slab with one straight 5 m wall and nothing else, so it is too impoverished to exercise the tile set. **For the kit generally the count is nine**: of the six wall-subsets of a cell up to rotation, three are covered (none, one face, two adjacent) and three are not — **corridor** (two opposite), **dead end** (three), **closet** (four); plus **window** (`wall_window`, 0.2 × 2.0), **wide doorway** (`wall_doorway_wide`, 0.4 × 2.0), the **shipped single doorway** (`wall_doorway`, 0.46 × 2.06 — FVS-R-10), **half-height** (`wall_low`), **structural corner** (`wall_corner`, 0.22 × 0.22), and a **void tile** that presents nothing *and* lays no floor, which nothing currently can because every tile carries a `site/floor` member.

  **Finding 2 — FVS-R-7's stated input does not exist.** The converted map yields **three adjacency pairs**. The decisions doc's *"#25 first because it is what produces more than four prototypes and real co-occurrence data"* has been struck and replaced (§1): #25 is worth doing for the representation change and the tile-gap census, explicitly **not** for the training data. A second map is deliberately out of scope — it needs three new tiles first, which trips `site_tiles.rs`'s `comps.len() == 4` and pre-empts FVS-R-6.

  **Finding 3 — the tool cannot lay a region of stamps, and FVS-R-14 is no longer cleanly separable from this item.** `Action::Fill` floods with `state.brush`, a *library descriptor index*, and `fill::flood` takes a `&Descriptor`; `CloneSet` holds a descriptor-shaped `ClonePiece` and cannot hold a stamp. So the tool built to author compositions has no verb that places more than one of them, and the conversion paid that 144 times. What is missing is not a convenience verb but the **semantic action layer**, which is FVS-R-14.

  **What the driving cost, recorded because none of it is visible from outside.** The slab is *wider than the strip between the two panels*, so the view has to be panned and the camera re-read per batch — a click landing on a panel is eaten by the `hovered_ui` gate and reports nothing. Panning is continuous while held, so it was driven as a closed loop against a **measured** response rather than by timing. Removal mode stayed armed after demolition and silently ate the first stamp, because `drive_place` refuses while another tool is live. And every batch was verified against the **saved file**, which is the only thing that knows what happened — the corpus vetting predicted exactly this brittleness for a coordinate-replay driver (Hernández-Bécares et al., `10.1016/j.entcom.2016.08.002`; Aroudj & Ostrowski, `s40601-013-0010-4`), and a screenshot at the end would not have caught twelve stamps landing one cell off. · *Deps:* — · *Reading:* [WFC]

- **FVS-R-6 — Is a lit wall a tag or an authored variant?** · S · ✅ **DECIDED 2026-08-10 — tags, shaped as Cooper's second layer**
  The last thing gating the schema work, and the corpus declined the binary it was posed as. `10.1609/aiide.v18i1.21944` (Cooper, *Sturgeon*) — already cited for expressive-range coverage — is the one system in the corpus that built this, and it **separates** the layers rather than choosing: a tile carries *"functional (i.e. gameplay) and/or image information"*, a level carries a *functional grid* and an *image grid*, and they can be generated *"sequentially by first generating a functional grid and then using that to limit image tile placement"* — which Cooper found the more efficient of the two. Tags are the join: *"the tile/tag distinction is intentionally blurred: functional tiles can be used as tags to constrain image tile placement."* A lit wall is appearance over a functional wall, so it is the second layer keyed to the first — not a second tile, and not an inert label.
  **The argument that decided it is about what survives a missing piece, not about the cross-product:** under tags an absent mesh is a missing renderer and the functional layer still solves; under variants the axis cannot be expressed at all. The Site kit has no wall-mounted piece, so under variants the sconce is not merely unauthored, it is **unauthorable** — the decision would have been blocked on an art task.
  **Shaped, not merely chosen.** The field is Sturgeon's image grid — a layer keyed to the functional tile — so *"a lit wall must face a room"* and *"never two adjacent"* stay expressible. Plain tags make that a schema change rather than an addition, and it is the same field either way, so the shape is free now and is not free later. **Only the field ships; the second solve waits for something that constrains it.** `Composition` carries no tag axis today (`composition.rs:87` — `id`, `envelope`, `members`, `locations`, `note`), so this costs FVS-R-8 **one field**, not a second prototype family — which is the cost that made this item gate the schema work.
  *Falsifier, recorded so this can be wrong later:* if the appearance layer ever needs an adjacency relation the functional layer cannot supply — a rule between two lit walls that does not reduce to a rule between the walls under them — the layers are not nested and the sequential form is the wrong shape. Nothing in the Site kit does this today; check it before writing the second solve, not after.
  *Written up:* `docs/research/2026-08-09-composition-grammar-decisions.md` §10. · *Reading:* Cooper 2022, `10.1609/aiide.v18i1.21944`

- **FVS-R-14 — What a tool does to a stamp** · L · ✅ **LANDED 2026-08-10**
  Stamped rows carry no `Placement` on purpose — *"so the remove, move and clone tools cannot see them at all"* — which was right while stamps were output-only and stopped being right when authoring moved to the Map. **The fix was never a component add.** A `Placement` on an expanded row makes every tool edit *rows*, which is the failure the omission prevented. What was missing is a **selectable identity for the instance**: one parent entity per stamp with the rows as children, so `Children`'s `linked_spawn` owns the lifetime and `Query<&Placement>` still correctly matches nothing — every existing tool unchanged by construction.
  *Shipped:* `StampInstance` + `StampPicture`; `pick_subject` running `pick_at`'s smallest-covering rule across loose pieces **and** stamped rows in one competition, keyed `(area, row id)` — total across both lists because `expand` refuses a reused stamp id, so its `<stamp>/<member>` rows cannot collide. **Delete** takes the instance; **move** grabs the whole stamp and writes `Stamped::at` (one field: the rows are derived, and the carried stamp hides through one `Visibility` write the rows inherit); **clone** carries a stamp beside its pieces and puts it down as a stamp; **capture** emits `Body::Composition { id }`, so a nested group is a reference and editing the inner composition still reaches it.
