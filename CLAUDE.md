...
Root package is the game (`foundation_vs_slop`, `src/`). Eighteen members live under `crates/`; sixteen mirror to `Ladvien/*` repos. **Nine are public** — `bevy_light_grid`, `bevy_speech_bubbles`, `bevy_orca`, `bevy_stigmergy`, `det_rng`, `map_elites`, and the three carved out of `bevy_carnage` on 2026-09-02: `bloodstain`, `bevy_wetmap` and `bevy_viscera` — each carrying a runnable example and, where the behaviour is only legible in motion, a gif. The rest stay private. **Four publish to crates.io: `bevy_carnage`, `bloodstain`, `bevy_wetmap`, `bevy_viscera`** — `bevy_carnage` cannot be published without them, because a released crate may not depend on a path-only dependency.
...
**Ten of those crates' examples also build for `wasm32-unknown-unknown` and are published as a demo site** at `https://ladvien.github.io/foundation_vs_slop/`, from this tree, because it is the only one where all four gore crates are visible to a single cargo workspace. `scripts/build_web.sh` builds it, `scripts/demo_facts.sh` holds the roster against `web/play.html`'s allow-list, and the `site`/`pages` jobs in `.github/workflows/ci.yml` deploy it. **Wasm demos never enable `vfx`**: `bevy_hanabi`'s wasm support is WebGPU-compute-only and every new visual is CPU-side anyway, so GPU particles stay a native extra.
