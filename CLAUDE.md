...
Root package is the game (`foundation_vs_slop`, `src/`). Twenty-two members live under `crates/`; twenty mirror to `Ladvien/*` repos. **Thirteen are public** — `bevy_light_grid`, `bevy_speech_bubbles`, `bevy_orca`, `bevy_stigmergy`, `det_rng`, `map_elites`, the three carved out of `bevy_carnage` on 2026-09-02: `bloodstain`, `bevy_wetmap` and `bevy_viscera`, and the four added on 2026-09-04: `bevy_cross_section`, `bevy_flaymap`, `bevy_laceration` and `bevy_fracture_modes` — each carrying a runnable example and, where the behaviour is only legible in motion, a gif. The rest stay private. **Eight publish to crates.io: `bevy_carnage`, `bloodstain`, `bevy_wetmap`, `bevy_viscera` and the four new ones** — `bevy_carnage` cannot be published without them, because a released crate may not depend on a path-only dependency.

**The 2026-09-02 carve-out, and why it was a de-duplication rather than a decomposition.** `bevy_carnage` held a blood model that had nothing to do with fracture: Carreau–Yasuda rheology, the Comiskey percolation spatter, pools, the bleed schedule, `hash_f32` and `WoundKind`. None of it needs Bevy and all of it is reusable, so it is now **`bloodstain`** — engine-free, `no_std`, and with **no math library in its public API** (`[f32; 3]` everywhere), which is the property that lets anything depend on it. This workspace resolves *two* `glam` versions, so a leaf naming either could collide with a consumer naming the other; naming neither cannot. `bevy_carnage` re-exports every moved name, and the conversion has exactly one home: `crates/bevy_carnage/src/v3.rs`, per wound rather than per vertex. That break is what took the crate to **0.2.0**.

`bevy_wetmap` and `bevy_viscera` are new rather than extracted. The first is texture-space blood accumulation that is **CPU-authoritative** — everywhere else this exists it is a GPU render target, which is why nobody can hash one — and the second is XPBD Cosserat-style strands with a tearing mesenteric membrane, fixed substep and iteration count so it can be a golden.

**The 2026-09-04 four, and the rule they were built under: each is a kernel `bevy_carnage` composes, and none may depend on it back** (every `tests/leaf.rs` names `bevy_carnage` as forbidden). `bevy_cross_section` bands a cut face by depth below the skin from a *sourced* per-region thickness table — dermis, subcutaneous fat, muscle, cortex, marrow, with the DOIs in `src/layers.rs` and cortical bone flagged as the one unsourced row — written into `UV_1` so `UV_0` and every golden that reads it are untouched. `bevy_flaymap` is `bevy_wetmap`'s shape applied to damage: a CPU depth buffer that peels those layers under repeated hits and returns a `Handoff` whose `bone_reached` is true on exactly one call per canvas — the caller turns that into the `BoneExposed` message, which the crate registers and never writes. `bevy_laceration` opens a cut on a time curve scaled by an authored tension and Ní Annaidh's Langer-line stiffness ratio, onto a bed the cross-section bands. `bevy_fracture_modes` is Sellán et al.'s *Breaking Good* reduced to the cell graph a convex decomposition already is — fixed-sweep Jacobi, fixed-step ICCM/ADMM, no convergence test anywhere — with one stated departure: the impact response divides each mode by its discontinuity energy. `bloodstain` 0.2 gained `spectral`: Bosschaart's whole-blood absorption and scattering tables under Kubelka–Munk and the CIE observer, so a film's colour is a function of thickness and oxygen saturation and `bevy_wetmap`'s coverage byte is now a depth. The bridges in `bevy_carnage` are `ProxyCell::skin_planes`, `FragmentGeometry::annotate_cap`, `FractureRegion`, `src/modal.rs` and `FractureCache::modes`.

**Fourteen of those crates' examples also build for `wasm32-unknown-unknown` and are published as a demo site** at `https://ladvien.github.io/foundation_vs_slop/`, from this tree, because it is the only one where all four gore crates are visible to a single cargo workspace. `scripts/build_web.sh` builds it, `scripts/demo_facts.sh` holds the roster against `web/play.html`'s allow-list, and the `site`/`pages` jobs in `.github/workflows/ci.yml` deploy it. **Wasm demos never enable `vfx`**: `bevy_hanabi`'s wasm support is WebGPU-compute-only and every new visual is CPU-side anyway, so GPU particles stay a native extra.
...
| `bevy_carnage` | Deterministic runtime gore: the slicer and watertight caps, the bake-once cache, bullet channels and their ejected plugs, **fracture that knows how it was loaded** (spiral/butterfly/transverse/comminuted, plus greenstick as an outcome), Grady's energy-driven fragment count, and the framework surface — `GoreTier`, the WCAG flash gate, the aim-exclusion cone, the decal budget. Wounds, impact-feel curves, decals and GPU blood sit behind its `vfx` feature | `crate::carnage` facade + `squad::{FigurineSource, GunModel}` aliases (`src/carnage.rs`) |
| `bloodstain` | **Blood as a material**, engine-free and `no_std`: Carreau–Yasuda rheology with a Casson yield stress, Comiskey percolation spatter, stain morphology derived from the impact, the six forensic pattern classes, the drying timeline, pools, the bleed schedule, and an inverse solver that recovers the wound from the stains. Public API is `[f32; 3]` | not used by the game directly — reached through `bevy_carnage`'s re-exports and `bevy_carnage::blood` |
| `bevy_wetmap` | Texture-space blood accumulation, **CPU-authoritative so it can be hashed** — two `Image`s per actor fed to a plain `StandardMaterial`, and a digest over the wet buffer | not used by the game yet; `WetmapPlugin` + `WetCanvas` |
| `bevy_viscera` | XPBD Cosserat-style strands with a tearing mesenteric membrane: guts that spill, coil, tether and tear. Fixed substep, iteration count and constraint order, so a tear is monotone and the solver is a golden | not used by the game yet; `VisceraPlugin` + `Strand`/`Mesentery` |
| `bevy_cross_section` | Anatomical bands on cut faces: a sourced per-region thickness table, exact depth-below-skin on a convex cell, `UV_1` annotation, and a procedural strip at physical scale with a frozen digest | through `bevy_carnage::cross_section`; `FractureRegion` on a subject bands its caps |
| `bevy_flaymap` | Texture-space flaying, CPU-authoritative: a depth buffer that peels skin→fat→muscle→cortex under hits and hands the bone off exactly once | through `bevy_carnage::flaymap`; `FlaymapPlugin` + `FlayCanvas` |
| `bevy_laceration` | A cut that gapes on a time curve scaled by skin tension and Langer-line orientation, onto a banded wound bed; kernel + plugin | through `bevy_carnage::laceration`; `LacerationPlugin` + `Laceration` |
| `bevy_fracture_modes` | Sellán's fracture modes on a cell graph: fixed-schedule bake, `A_i` impact rows, partition by gluing tolerance. Registers no systems | through `bevy_carnage::fracture_modes` and `bevy_carnage::modal`; `FractureCache::modes` |
