...
**It still does not mirror back, and the reason changed.** The old reason was that a `subtree split` carries only commits, so the crate's then-uncommitted audit harness could never reach the published repository. The live reason is narrower and harder: the two histories are **unrelated** — none of AG-025..AG-031 ever existed here — so a split push to `Ladvien/bevy_carnage` would not fast-forward, and this repo never forces one (see `scripts/mirror_crates.sh`'s header). So it stays absent from `CRATES` on purpose, and **the monorepo is now its source of truth**. It is also no longer the one crate outside `cargo test --workspace`: its three frozen goldens — `fracture_output_is_bit_identical_across_runs`, `hash_f32_is_frozen`, `the_spatter_model_is_frozen` — are inside the gate for the first time.
...
| `bevy_carnage` | Deterministic runtime gore: the slicer and watertight caps, the bake-once cache, bullet channels and their ejected plugs, and (unused here so far) wounds, spatter, bleed schedule, impact-feel curves, decals and GPU blood behind its `vfx` feature | `crate::carnage` facade + `squad::{FigurineSource, GunModel}` aliases (`src/carnage.rs`) |
