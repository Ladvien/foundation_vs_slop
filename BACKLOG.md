...
**FVS-R-17 · `Empty` is the universal escape, and it is why nothing closes.** Measured by FVS-R-9, not felt: over 128 solves `Empty` asked for **20%** of cells by weight and took **37.58%**, while every authored tile came in under its ask. It is the one prototype compatible with every neighbour — deliberately, since *"a grammar that cannot say 'nothing goes here' cannot leave a doorway"* — so propagation never eliminates it while every wall turn is eliminated somewhere, and every wall run terminates into it on its first step. **The room is legal and the solver simply never finds it:** `the_kit_can_build_a_room_the_metric_calls_enclosed` lays one by hand from the kit's own prototypes and asserts every adjacent pair is permitted by the learned support. So this is not a vocabulary problem, and authoring FVS-R-5's nine missing kinds does not touch the mechanism. Sandhu, Chen & McCoy (`10.1145/3337722.3337752`) name the two terms: *"constraints that can work over any distance"* and *"weight recalculation"*. A closed boundary is a property over distance and plain WFC has no term for it. *Done when:* a change to `Empty`'s weight or role, or a distance constraint, is re-measured against §4's **same committed rows** and the histogram is no longer empty. · *Reading:* `10.1145/3337722.3337752`, `docs/research/2026-08-10-expressive-range.md`
