...
- ~~**A comma cannot be a key chord in `emerge-mapper`.**~~ ✅ **RESOLVED 2026-09-04.** `keys::chord_column` joined a collapsed row's chords with `", "`, so a comma chord was unreadable the moment it shared a row with anything — the member walk's `,`/`.` badge rendered as `",, ."`. The separator is a mid-dot now (` · `), `collapsing_rows_loses_nothing` sweeps `Context::ALL × Stance::ALL` and splits on whitespace, and the constraint is gone. `crates/emerge-mapper/docs/kit_review.md` Part 4, N15.
