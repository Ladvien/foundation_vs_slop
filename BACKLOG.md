...
**FVS-R-15 · Compose becomes read-only, and authoring lives on the Map.** Compose was reimplementing — worse — the spatial tools the Map already has: box-select, clone, move, fine placement, undo. Arrange freely on the Map, box-select, name it, and it is a composition; arrange *those* and capture again, which is `Body::Composition { id }` at depth ≤ 8. **Call it nesting, never "subgrid"** — `Descriptor::subgrid` is the per-mesh edge-token lattice, and `seating_divisions`/`face_bands` were split apart for exactly this reason; a third claimant retires the word for new things. Cut the add-piece verb and the PLACE list, seat, flush, turn, drop, paint, `N`, and undo — eight of twelve rows, leaving walk / arm / view. **`R` moves too, and that is the point:** it is derivation rather than authoring, which makes it feel like a view operation, but it persists — leave it and the property becomes "read-only except `R`", which is a description rather than an invariant. The invariant being bought is **one writer**: nothing on this tab opens `compositions.ron`. **Cheaper than it reads** — the rollback cuts *bindings*, not the tested core: `seat`, `flushed`, `turned_footprint` and `member_footprint` are pure and `site_tiles.rs` pins them through the data, so none of it cares which tab calls it. **Flush can be cut — measured, not assumed.** It existed because Compose only had a lattice and −0.45 is off it. `editor::map_at` clamps free placement to the cell the modifier went down in — *"anywhere inside that half-metre, nowhere outside it"* — and the cell around a snapped −0.5 spans [−0.75, −0.25], so −0.45 is reachable on the Map in two gestures. The wall tiles `site_tiles.rs` pins stay authorable. **Capture writes to the loaded kit** — one path; a scratch kit plus a promote step is two. The shipped-asset hazard largely dissolves with `N`, which is what put `test` and `testing` into `assets/emerge/site/compositions.ron` in one keypress. *Done when:* Compose opens `compositions.ron` nowhere — pinned by `tests/compose_is_read_only.rs`, a source ratchet on the precedent of `census_is_the_one_counter.rs`. **The "nested capture round-trips" clause was moved to FVS-R-14, where it belongs:** nesting needs `CloneSet` to hold a stamp, which the rollback neither provides nor needs, and leaving it here made this item unfinishable against a condition it was never going to satisfy.
...
**FVS-R-8 · The schema: typed split values, the occupancy test, and the tag axis.** **The "real solve" gate is met** (FVS-R-9 ran 2026-08-10; the solve converges every time, it just does not make rooms — see FVS-R-17), so this is unblocked. Originally: only after a real solve — **FVS-R-6 is decided** (2026-08-10: tags, shaped as Cooper's second layer; archived, and `docs/research/2026-08-09-composition-grammar-decisions.md` §10). Absolute/relative split values (CGA); the occupancy test **with `Shape.occ("noparent")` scoping in the same commit**; and **one field on `Composition`** for the tag axis — `composition.rs:87` carries none today, and the decision costs exactly that field, not a second prototype family. **The tag field is Sturgeon's image grid, not a renderer hint** — a layer keyed to the functional tile, so an adjacency rule over the lit axis stays expressible; the sequential solve itself waits for something that constrains it, and building it now would be building against a guess. **Do not retire the positional `Mount` variants** — furniture uses them and furniture is deliberately not gridded. This is where a golden is expected to move.

**FVS-R-19 · A latched rung, not only a held one.** The placement ladder (`grid::SnapLevel`, shipped 2026-08-10) is driven by held modifiers — Shift for one rung finer, Cmd+Shift for two. **Both snapping papers latch their modes instead.** Bier's snap-dragging changes gravity mode with keyboard commands (*"Cycle Forward (Backward) through the Three Gravity Functions, Toggle Gravity On and Off"*) and holds nothing; StickyLines' designers say why — they *"make extensive use of the keyboard… not only because it is faster, but also because 'there are too many options and menus' that clutter their screens and make them 'lose focus'"* (`10.1145/2984511.2984577`). Holding Shift is right for one nudge and wrong for a dressing session, which is the case the lower rungs exist for. **The verb already exists:** `CycleGrid` cycles the drawn grid, and since `a10cadf` the drawn grid IS the active rung — so cycling the rung and cycling the grid are now one act. Keep the modifiers as a transient override, which is the shape Bier's gravity toggle already has. *Done when:* an author can set a rung and let go. · *Reading:* `docs/research/2026-08-10-snapping-corpus-vetting.md` §2

**FVS-R-20 · The geometric box is not the visual one, and snapping uses the wrong one.** `brush_span` snaps by `extent.footprint`, the measured box. StickyLines names this exactly: *"Alignment and distribution commands use the geometric center of objects, but sometimes this does not match the object's visual center… **All were forced to fine-tune the result**… To our knowledge, current tools completely ignore such tasks"* (`10.1145/2984511.2984577`). **This repo already met it and wrote it down without naming it** — `policy.rs` on seating: *"`site/wall` is 0.1 m thick and sits flush at −0.45, which is not a multiple of 0.125 either, because art is authored to look right rather than to tile."* So a piece whose art does not fill its box snaps arithmetically right and visually wrong, and the only recourse is Alt free-placement, which discards the lattice entirely. **The fix is a snap box, not a snap rule:** an authored per-descriptor offset/extent used for snapping only, beside `align.pivot` and `align.y_offset`, which are already "how this art sits" fields — the cheaper, kit-shaped half of StickyLines' *tweak the bounding box… without affecting the object itself*. *Done when:* a piece whose art is inset from its box lands where it looks right, with no free placement. · *Reading:* `docs/research/2026-08-10-snapping-corpus-vetting.md` §3

**FVS-R-17 · Local adjacency cannot close a boundary, and that is now measured rather than argued.** FVS-R-9 found the generator produces **zero enclosed regions in 128 solves**. The first diagnosis was `Empty`'s weight — it asks for 20% of cells and takes 37.58%, being the one prototype compatible with every neighbour. **That hypothesis is FALSIFIED** (`examples/empty_weight.rs`, 2026-08-10): sweeping the weight from 1.0 to **0.0** tracks its cell share exactly (37.6% → 0.0%) and moves enclosed regions from 0 to **2 of 128**, with median enclosure never leaving 0.000. Deleting `Empty` from the output entirely does not make rooms. Nor is the alphabet over-constrained — row 3 never fires, every solve converges even when every cell must be an authored tile. **So it is not vocabulary (FVS-R-5's nine missing kinds), not weight, and not over-constraint: it is the solver's expressiveness.** A closed boundary is a property *over distance* — a wall run must know it will meet a corner that meets a wall that returns — and local pairwise support cannot express that, which is why no reweighting of local choices produces it. That is the first of the two terms Sandhu, Chen & McCoy name, *"constraints that can work over any distance"*; the sweep rules out their second, *"weight recalculation"*, for this failure. *Done when:* a change that adds a constraint over distance is re-measured against §4's **same committed rows** and the histogram is no longer empty. · *Reading:* `10.1145/3337722.3337752`, `docs/research/2026-08-10-expressive-range.md` §7

**FVS-R-18 · An empty histogram wants to be its own outcome.** FVS-R-9's run exposed two blind spots in §4, recorded rather than fixed because amending a criterion after seeing its output is what §4 forbids. The stopping rule was satisfied by two **empty** blocks agreeing — total variation between empty histograms is 0 by definition, so it read as convergence — and rows 4a and 4b disagree on an empty histogram while neither says anything about concentration. *Done when:* both are **pre-registered** as an amendment before the next expressive-range run, not after it.
