...
**FVS-R-12 · `bevy_debugger/input` cannot type — NEXT, at three strikes in one day.** It writes `ButtonInput` but not the `KeyboardInput` message stream, so no agent can drive a text field. **Blocked three separate verifications on 2026-08-09**: naming a composition on Compose, naming one captured on the Map, and now replace-on-capture — which is the one thing in that batch that cannot be signed off without a human at the keyboard. At three occurrences in one session the workaround cost has passed the fix cost. The crate is ours (`crates/bevy_debugger_mcp/`); a `kind: "Text"` emitting `Key::Character` events is the shape, and cursor injection landed the same day as the precedent. *Done when:* an agent can name a composition end to end.
