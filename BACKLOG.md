...
**FVS-R-14 · What a tool does to a stamp — decided once, before three features decide it separately.** Stamped rows carry **no `Placement`**, deliberately: *"so the remove, move and clone tools cannot see them at all."* That was right while stamps were output-only, and moving authoring to the Map is what invalidates it. Three features now want in — pin a stamp (FVS-R-7's fixed tiles), box-select a stamp (nesting), and any Map-side edit of a placed composition. **It is not a component add.** Giving stamped rows the loose-piece `Placement` makes every tool edit *expanded rows*, which is the failure the omission prevented. What is missing is a **selectable identity for the stamp as a unit** — a parent entity per stamp with the rows as children, so `ChildOf`'s `linked_spawn` owns the lifetime and `Placement` queries still correctly see nothing. **The three answers then fall out of one rule already in `composition.rs`'s header** — USD-style encapsulation, *"a stamp may override values, never delete/re-parent/reach into a nested composition"*: Delete removes the **instance**; move grabs the **whole stamp** and writes `Stamped::at`; clone makes a **second stamp**, because copying expanded rows flattens the reference the schema exists to protect (`stamping_writes_a_reference_and_undo_takes_it_back` pins it). One rule: **tools act on the instance, never through it.** The real work is `CloneSet`, which holds a descriptor-shaped `ClonePiece` and must hold *either* a piece or a stamp before `composition_from_set` can emit `Body::Composition { id }` — which is where nesting closes. **FVS-R-5 promoted this from "waiting on a trigger" to "the thing that would have made FVS-R-5 mechanisable"** — converting `site_67` needed 144 individual stamps because no verb places a region of them, and `Action::Fill` floods with a *descriptor* brush that `CloneSet` cannot hold a stamp for. The missing piece is the semantic action layer, not a convenience verb. *Done when:* the rule is written down, `CloneSet` carries both, **a nested capture round-trips** (moved here from FVS-R-15), and delete/move/clone each act on the instance. · *Blocks:* FVS-R-7's fixed tiles, and nesting.
