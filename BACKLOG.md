...
- **⚠️ Two harness oracles are red, pre-existing, and nothing gates on them (measured 2026-08-05).** Confirmed by running `tests/replay.rs` from a worktree at `aea728b` and getting the identical result, so neither belongs to the `feat/emerge-lattice` work. The harness lane is `continue-on-error`, which is how both stayed unnoticed.
  ...
  **`photophobia_pulls_crabs_into_shadow`** is *inverted*, not merely unmet: `photophobic crabs (gain>0) should occupy darker cells than gain=0 crabs: on=0.195 off=0.114` — the group with light-avoidance on ends up in **brighter** cells than the control. Either the gain is wired with the wrong sign, the oracle reads the light field with the wrong polarity, or the control group is not a control. `src/light.rs`'s `Photophobic` is the subject; see the lighting-status notes for the toolkit.
