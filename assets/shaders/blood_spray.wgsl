...
// Hash-based value noise in [0,1) (Dave Hoskins style), texture-free stand-in for a noise sampler.
...
// A pseudo-random launch direction for droplet `i`, roughly filling the unit disc.
fn rand_dir(i: f32, seed: f32) -> vec2<f32> {
    let angle = hash21(vec2<f32>(i, seed)) * 6.2831853;
    let radius = 0.25 + 0.75 * hash21(vec2<f32>(i + 11.0, seed + 3.0));
