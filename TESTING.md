...
10. **Killing units early races the async fracture bake — settle it, at a FIXED tick.** `bevy_autogib::bake_fractures`
...
| `crates/bevy_autogib/src/{soup,mesh}.rs` | Fracture **topology** — slice/cap geometry, reaches target & deterministic, missing UV/normals synthesized, open-boundary dropped, degenerate plane leaves piece whole, the frozen `hash_f32` bits. (Counts/structure, never float vertex positions.) Reached by `cargo test --workspace`, **not** by bare `cargo test` — see the note at the top of this file. |
...
| `tests/rng_guard.rs` | GPU-free (no feature) | Freezes the exact bit output of every generator — `util` (`next_u32`, `rand01`, `hash01_u32`, `hash_f32`) and `rng::seeded` ChaCha8 (`raw_u64`, `unit`, `below`). A silent constant change trips here first. Also asserts `util::hash_f32` and `bevy_autogib::hash_f32` agree: a leaf crate cannot import the game's, so the fracture RNG is two copies of one generator and this is what stops them drifting. |
