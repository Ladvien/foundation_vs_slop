...
10. **Killing units early races the async fracture bake — settle it, at a FIXED tick.** `bevy_carnage::bake_fractures`
...
| `bevy_carnage`'s `src/{soup,mesh}.rs` | Fracture **topology** — slice/cap geometry, reaches target & deterministic, missing UV/normals synthesized, open-boundary dropped, degenerate plane leaves piece whole, the frozen `hash_f32` bits. (Counts/structure, never float vertex positions.) Reached by `cargo test --workspace`, **not** by bare `cargo test` — see the note at the top of this file. |
